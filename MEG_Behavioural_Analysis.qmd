---
title: "MEG_Behavioural_Analysis"
format: html
editor: visual
...
```{r}
data = read_csv(file = 'MEG_Behavioural_data.csv')
```
...
```{r}
report(BAN_ACC_model_1)
```

```{r}
report_statistics(BAN_ACC_model_1)
```

```{r}
report(BAN_RT_model_1)
```

```{r}
report_statistics(BAN_RT_model_1)
```
